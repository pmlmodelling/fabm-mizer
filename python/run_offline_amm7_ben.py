...
stop_time = datetime.datetime(2002, 1, 1)
...
#long name, ERSEM name, esd, is benthos?
preylist.append(('diatoms', 'P1_c', esd2mass(20., 200.), False))
preylist.append(('nanophytoplankton', 'P2_c',   esd2mass(2., 20.),False))
preylist.append(('picophytoplankton', 'P3_c',   esd2mass(.2, 2.),False))
preylist.append(('microphytoplankton', 'P4_c', esd2mass(20., 200.), False))
preylist.append(('mesozooplankton', 'Z4_c', (4.188e-6, 1e-3),False))
preylist.append(('microzooplankton', 'Z5_c', esd2mass(20., 200.),False))
preylist.append(('heterotrophic nanoflagellates', 'Z6_c', esd2mass(2., 20.),False))
preylist.append(('detrital feeders', 'Y3_c', esd2mass(2., 20.),True))
preylist.append(('deposit feeders', 'Y2_c', esd2mass(20., 200.),True))
temp_name = 'votemper'
time_name = 'time_counter'
omega_name = 'omega'
bot_temp_name= 'votemper_bot'
#ben_preylist=[]
#ben_preylist.append(('detrital feeders', 'Y3_c', esd2mass(2., 20.)))
#ben_preylist.append(('deposit feeders', 'Y2_c', esd2mass(20., 200.)))
bot_temp_name = 'votemper_bot'




#Array of whether prey is pelagic or benthic
#prey_is_pel= numpy.ones(len(preylist))
#prey_is_ben= numpy.zeros(len(preylist))
#prey_is_pel[-2:] = 0
#prey_is_ben[-2:] =  1
...
def processLocation(args):
    ...
    prey = []
    prey_is_pel,prey_is_ben = [],[]
    for name, ncname, size_range, isben in preylist:
        ...
        if isben==True:
            ...
            prey_is_pel=1
            prey_is_ben=0
        prey.append(mizer.Prey(name, size_range, timeseries, prey_is_pel, prey_is_ben))
...
#    ben_prey = []
#    for name, ncname, size_range in ben_preylist:
#        timeseries = mizer.datasources.TimeSeries(path, ncname, scale_factor=scale_factor, time_name=time_name, x=i, y=j, stop=stop_time)
#        times = timeseries.times
#        ben_prey.append(mizer.Prey(name, size_range, timeseries))
#    
#    #data for combned pelagic and benthic prey
#    combined_prey = prey.extend(ben_prey)
#    prey_collection = mizer.PreyCollection(*combined_prey)
#    
#    #benthic prey only
#    ben_prey_collection = mizer.PreyCollection(*ben_prey)
#    ben_prey_collection = mizer.GriddedPreyCollection(ben_prey_collection)
#    
#    #pelagic prey only
#    pel_prey_collection = mizer.PreyCollection(*prey)
#    pel_prey_collection= mizer.GriddedPreyCollection(pel_prey_collection)
    ...
    ben_temp =  mizer.datasources.TimeSeries(path, bot_temp_name, time_name=time_name, x=i, y=j, stop=stop_time)
    
    #Amount of time fish spend on feeding in the pelagic vs benthos calulated from pelagc prey:total prey ratio
    omega =  mizer.datasources.TimeSeries(path, omega_name, time_name=time_name, x=i, y=j, stop=stop_time)
    #temp = 12.

    # create mizer model
#    m = mizer.Mizer(prey=prey_collection,parameters=parameters, ben_prey=ben_prey_collection,  temperature=temp,  ben_temperature = ben_temp, omega=omega, recruitment_from_prey=2, depth=depth))
    m = mizer.Mizer(prey=prey_collection, parameters=parameters,temperature=temp,  ben_temperature = ben_temp, omega=omega, recruitment_from_prey=2, depth=depth)
...
if __name__ == '__main__':
        ...
        if source not in source2output:
           ...
           if args.ifirst is not None:
              ...
              with netCDF4.Dataset(path) as nc:
                ...
                vardict['bio1_25'] = addVariable(ncout, 'bio1_25', 'fish WM > 1.25 g', 'g WM/m2', dimensions=(time_name, 'y', 'x'), zlib=compress, contiguous=contiguous)
                vardict['bio10'] = addVariable(ncout, 'bio10', 'fish WM > 10 g', 'g WM/m2', dimensions=(time_name, 'y', 'x'), zlib=compress, contiguous=contiguous)
                vardict['bio80'] = addVariable(ncout, 'bio80', 'fish WM> 80 g', 'gWM/m2', dimensions=(time_name, 'y', 'x'), zlib=compress, contiguous=contiguous)                
                vardict['bio1000'] = addVariable(ncout, 'bio1000', 'fish WM > 1000 g', 'gWM/m2', dimensions=(time_name, 'y', 'x'), zlib=compress, contiguous=contiguous)
    ...
    def saveResult(result, sync=True, add_biomass_per_bin=False):
        ...
        if add_biomass_per_bin:
           ...
           vardict['Nw_start'][j,i,:]=spectrum[0,:]
        if sync:
