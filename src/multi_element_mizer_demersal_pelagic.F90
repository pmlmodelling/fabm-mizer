...
! !PUBLIC DERIVED TYPES:
   type,extends(type_particle_model),public :: type_multi_element_demersal_pelagic_population
      ...
      type (type_horizontal_diagnostic_variable_id)             :: id_total_reproduction_pel ! Total reproduction
      type (type_horizontal_diagnostic_variable_id)             :: id_R_p_pel                ! Density-independent recruitment
      type (type_horizontal_diagnostic_variable_id)             :: id_R_pel                  ! Density-dependent recruitment
      ...
      real(rk) :: recruitment_pel,  recruitment_ben! constant recruitment flux (SSR=0)
...
!BOC
   ...
   case (0)
      call self%get_parameter(self%recruitment_pel, 'recruitment_pel', '# m-2 yr-1', 'constant recruitment flux', minimum=0.0_rk, default=kappa*self%w_min**(-lambda)*sec_per_year, scale_factor=1._rk/sec_per_year)
   ...
   call self%get_parameter(self%omega,      'omega',  '-',    'fraction of time that fish spend in the pelagic',  minimum=0.0_rk, maximum=1.0_rk) !default=1.0_rk,
   ...
   allocate (self%id_fprey_c(self%nbenprey ))
   allocate (self%id_fprey_n(self%nbenprey ))
   allocate (self%id_fprey_p(self%nbenprey))
   allocate (self%id_fprey_s(self%nbenprey))
   ...
   do iprey=1,self%nprey
      ...
      !     call self%request_coupling_to_model(self%id_pelprey_c(iprey), 'prey'//trim(strindex), standard_variables%total_carbon)    
          ! call total_pelprey_calculator%add_component('pelprey_c'//trim(strindex)) 
           call self%register_horizontal_dependency(self%id_benprey_c(b), 'benprey_c'//trim(strindex2), 'mmol C m-3', 'carbon in benthic prey '//trim(strindex2)) 
 ...
 !      call depth_averaged_prey%set_variable_property(depth_averaged_prey%id_c, 'max_particle_mass', w_prey_max)          
           ...
           call self%register_diagnostic_variable(self%id_fprey_c(b),'fprey'//trim(strindex)//'c','mmol C/m^2/d',   'uptake of carbon in food source '//trim(strindex),    source=source_do_bottom)
           call self%register_diagnostic_variable(self%id_fprey_n(b),'fprey'//trim(strindex)//'n','mmol C/m^2/d',   'uptake of nitrogen in food source '//trim(strindex),    source=source_do_bottom)
           call self%register_diagnostic_variable(self%id_fprey_p(b),'fprey'//trim(strindex)//'p','mmol C/m^2/d',   'uptake of phosphorus in food source '//trim(strindex),    source=source_do_bottom)
           call self%register_diagnostic_variable(self%id_fprey_s(b),'fprey'//trim(strindex)//'s','mmol C/m^2/d',   'uptake of silicon in food source '//trim(strindex),    source=source_do_bottom)
...
!      call depth_averaged_class_demersal%set_variable_property(depth_averaged_class_demersal%id_c,'particle_mass',self%w(iclass))
      ...
      if (self%SRR == 1 .or. self%SRR == 2) call self%register_diagnostic_variable(self%id_reproduction_pel(iclass),'reproduction_pel'//trim(strindex),'g m-2 d-1','allocation to reproduction in size class '//trim(strindex),         source=source_do_bottom)
   ...
   call self%register_state_dependency(self%id_Bendip,'dipp','mmol P/m^3','disssolved inorganic phosphorus')
   call self%register_state_dependency(self%id_Bendin,'dinn','mmol N/m^3','dissolved inorganic nitrogen')
   ...
   call self%register_state_dependency(self%id_Bendic,'O3c','mmol C/m^3','carbon dioxide')
   call self%register_state_dependency(self%id_Beno2,'O2o','mmol O_2/m^3','oxygen')
   call self%request_coupling_to_model(self%id_Bendic,   'Benthic_respired_carbon',   standard_variables%total_carbon)
   call self%request_coupling_to_model(self%id_Beno2,   'Benthic_oxygen_consumed',   standard_variables%total_carbon)
   ...
   if (self%SRR == 1 .or. self%SRR == 2) then
      call self%register_diagnostic_variable(self%id_total_reproduction_pel,'total_reproduction_pel','mmol C m-2 d-1','total pelagic reproduction',source=source_do_bottom)
      ...
      call self%register_diagnostic_variable(self%id_R_p_pel,'R_p_pel','# m-2 d-1','density-independent recruitment',source=source_do_bottom)
...
!            self%V = self%V * g_per_mmol_carbon  ! express volumetric search rate per mmol carbon (instead of per g)         
   ...
   subroutine do_bottom(self,_ARGUMENTS_DO_BOTTOM_)
      ...
      real(rk) :: c_lfi, c_size1,c_size2, c_size3, slope, offset, endpoint_pel, expected_eggs_pel,benslope, benoffset, endpoint_ben, expected_eggs_ben
      real(rk) :: total_reproduction_pel,total_reproduction_ben,T_lim,T_lim_bot,temp,bot_temp,T_w_int,w_int,g_tot_pel, g_tot_ben
      real(rk) :: R_pel,R_ben,R_p_pel, R_p_ben
      ...
      real(rk),dimension(self%nclass) :: mu_pel,reproduction_pel,maintenance_pel,g_pel,maintenance_ben,g_ben,mu_ben,reproduction_ben
      ...
      _HORIZONTAL_LOOP_BEGIN_
         ...
         do iprey = 1, self%nprey
            if (iprey > self%nprey - self%nclass) then
               ...
               _SET_BOTTOM_ODE_(self%id_prey_c(iprey), -(prey_loss_pel(iprey)*prey_c(iprey)) -(1._rk-self%omega)*prey_loss_ben(iprey)*prey_c(iprey))
            ...
            elseif (self%feedback) then
               ! Prey is an external variable (not one of our size classes)
                   if (self%prey(iprey)%isben) then
                       ...
                       _SET_BOTTOM_ODE_(self%id_prey_c(iprey), -self%omega*prey_loss_pel(iprey)*prey_c(iprey))
                       _SET_BOTTOM_ODE_(self%id_prey_n(iprey), -self%omega*prey_loss_pel(iprey)*prey_n(iprey))
                       _SET_BOTTOM_ODE_(self%id_prey_p(iprey), -self%omega*prey_loss_pel(iprey)*prey_p(iprey))
                       _SET_BOTTOM_ODE_(self%id_prey_s(iprey), -self%omega*prey_loss_pel(iprey)*prey_s(iprey))                                      
                   end if
            end if
         end do
       ...
       !   PRINT*, omega
        !  _SET_HORIZONTAL_DIAGNOSTIC_(self%id_test,(omega))
         ...
         do iclass=1,self%nclass
            ...
            _SET_BOTTOM_ODE_(self%id_c(iclass),(-(mu_pel(iclass) + self%F(iclass))*Nw(iclass) + (nflux_pel(iclass-1)-nflux_pel(iclass))*self%w(iclass)/g_per_mmol_carbon)*self%omega)
            _SET_BOTTOM_ODE_(self%id_c(iclass),(-(mu_ben(iclass) + self%F(iclass))*Nw(iclass) + (nflux_ben(iclass-1)-nflux_ben(iclass))*self%w(iclass)/g_per_mmol_carbon)*(1._rk-self%omega))
         ...
         if (self%SRR == 1 .or. self%SRR == 2) then
            _SET_HORIZONTAL_DIAGNOSTIC_(self%id_total_reproduction_pel,total_reproduction_pel*86400)
            _SET_HORIZONTAL_DIAGNOSTIC_(self%id_total_reproduction_ben,total_reproduction_ben*86400)
            _SET_HORIZONTAL_DIAGNOSTIC_(self%id_R_p_pel,R_p_pel*86400)
            _SET_HORIZONTAL_DIAGNOSTIC_(self%id_R_p_ben,R_p_ben*86400)
         end if
         _SET_HORIZONTAL_DIAGNOSTIC_(self%id_R_pel,R_pel*86400)
         ...
         if (self%feedback) then
            _SET_BOTTOM_ODE_(self%id_o2,-self%omega*self%resp_o2C*(1-self%alpha-self%alpha_eg)*sum(I_c_pel*Nw))            
            _SET_BOTTOM_ODE_(self%id_dic,self%omega*(1-self%alpha-self%alpha_eg)*sum(I_c_pel*Nw))
            _SET_BOTTOM_ODE_(self%id_din,self%omega*(1-self%alpha-self%alpha_eg)*sum(I_n_pel*Nw))
            _SET_BOTTOM_ODE_(self%id_dip,self%omega*(1-self%alpha-self%alpha_eg)*sum(I_p_pel*Nw))
            _SET_BOTTOM_ODE_(self%id_waste_c,(sum(((self%alpha+self%alpha_eg)*I_c_pel +  mu_pel - g_pel/(1-self%psi)          )*Nw) - R_pel*self%w_min/g_per_mmol_carbon          + nflux_pel(self%nclass)*(self%w(self%nclass)+self%delta_w(self%nclass))/g_per_mmol_carbon)*self%omega)
            _SET_BOTTOM_ODE_(self%id_waste_n,(sum(((self%alpha+self%alpha_eg)*I_n_pel + (mu_pel - g_pel/(1-self%psi))*self%qnc)*Nw) - R_pel*self%w_min/g_per_mmol_carbon*self%qnc + nflux_pel(self%nclass)*(self%w(self%nclass)+self%delta_w(self%nclass))/g_per_mmol_carbon*self%qnc)*self%omega)
            _SET_BOTTOM_ODE_(self%id_waste_p,(sum(((self%alpha+self%alpha_eg)*I_p_pel + (mu_pel - g_pel/(1-self%psi))*self%qpc)*Nw) - R_pel*self%w_min/g_per_mmol_carbon*self%qpc + nflux_pel(self%nclass)*(self%w(self%nclass)+self%delta_w(self%nclass))/g_per_mmol_carbon*self%qpc)*self%omega)
            _SET_BOTTOM_ODE_(self%id_waste_s,sum(I_s_pel*Nw)*self%omega)
            
            _SET_BOTTOM_EXCHANGE_(self%id_beno2,-(1._rk-self%omega)*self%resp_o2C*(1-self%alpha-self%alpha_eg)*sum(I_c_ben*Nw))            
            _SET_BOTTOM_EXCHANGE_(self%id_bendic,(1._rk-self%omega)*(1-self%alpha-self%alpha_eg)*sum(I_c_ben*Nw))
            _SET_BOTTOM_EXCHANGE_(self%id_bendin,(1._rk-self%omega)*(1-self%alpha-self%alpha_eg)*sum(I_n_ben*Nw))
            _SET_BOTTOM_EXCHANGE_(self%id_bendip,(1._rk-self%omega)*(1-self%alpha-self%alpha_eg)*sum(I_p_ben*Nw))
            _SET_BOTTOM_EXCHANGE_(self%id_bendiscard_c,(sum(((self%alpha+self%alpha_eg)*I_c_ben +  mu_ben - g_ben/(1-self%psi)          )*Nw) - R_ben*self%w_min_ben/g_per_mmol_carbon          + nflux_ben(self%nclass)*(self%w(self%nclass)+self%delta_w(self%nclass))/g_per_mmol_carbon)*(1._rk-self%omega))
            
            _SET_BOTTOM_EXCHANGE_(self%id_bendiscard_n,(sum(((self%alpha+self%alpha_eg)*I_n_ben + (mu_ben - g_ben/(1-self%psi))*self%qnc)*Nw) - R_ben*self%w_min_ben/g_per_mmol_carbon*self%qnc + nflux_ben(self%nclass)*(self%w(self%nclass)+self%delta_w(self%nclass))/g_per_mmol_carbon*self%qnc)*(1._rk-self%omega))
            
            _SET_BOTTOM_EXCHANGE_(self%id_bendiscard_p,(sum(((self%alpha+self%alpha_eg)*I_p_ben + (mu_ben - g_ben/(1-self%psi))*self%qpc)*Nw) - R_ben*self%w_min_ben/g_per_mmol_carbon*self%qpc + nflux_ben(self%nclass)*(self%w(self%nclass)+self%delta_w(self%nclass))/g_per_mmol_carbon*self%qpc)*(1._rk-self%omega))
            
            _SET_BOTTOM_EXCHANGE_(self%id_bendiscard_s,sum(I_s_ben*Nw)*(1._rk-self%omega))
