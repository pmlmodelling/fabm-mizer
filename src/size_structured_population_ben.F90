...
module mizer_size_structured_population_ben
...
! at the centre of the bin. Note that each division roughly corresponds to a decrease of 1 in spectrum slope.
   ...
   type, extends(type_size_structured_base), public :: type_size_structured_population_ben
   ...
   end type type_size_structured_population_ben
...
!
! !INPUT PARAMETERS:
   class (type_size_structured_population_ben), intent(inout),target :: self
...
!BOC
   ...
   case (1)
      ...
      call self%register_dependency(self%id_T_b, 'T_b', 'degrees_Celsius', 'bottom temperature for demersal fish')
   end select
   ...
   !Add in paramter for time spent in bentos
   call self%register_dependency(self%id_omega, 'omega', '-', 'fraction of time that fish spend in the pelagic')  
  ...
  call self%get_parameter(self%omega_size,'omega_size','-',          'use of size dependant omega', default=.false.)
   ...
   allocate(self%id_pelspec(self%nprey))
   allocate(self%id_benspec(self%nprey))
   do iprey = 1, self%nprey
       write (strindex,'(i0)') iprey
       call self%register_dependency(self%id_pelspec(iprey), 'pelspec'//trim(strindex), '-', 'Can prey be pelagic 1=True, 0 =False') 
...
!EOC
   ...
   subroutine after_coupling(self)
      class (type_size_structured_population_ben),intent(inout) :: self
   ...
   subroutine do_bottom(self,_ARGUMENTS_DO_BOTTOM_)
      class (type_size_structured_population_ben),intent(in) :: self
      ...
      _HORIZONTAL_LOOP_BEGIN_
         ...
         !Retrieve arrays for pelagic and benthic prey
         do iprey=1,self%nprey
             _GET_(self%id_pelspec(iprey), pelspec(iprey))
             _GET_(self%id_benspec(iprey), benspec(iprey))
         end do
         ...
         do iclass=1,self%nclass
            ...
            E_e_ben = self%VB(iclass)*E_a_ben ! Eq M3
            ...
            prey_loss_pel(:) = prey_loss_pel(:) + I_pel(iclass)/E_a_pel*self%phi(:,iclass)*Nw(iclass)*omega(iclass)
            if (omega(iclass)/=1) then
                prey_loss_ben(:) = prey_loss_ben(:) + I_ben(iclass)/E_a_ben*self%phi(:,iclass)*Nw(iclass)*(1-omega(iclass))
            else
         ...
         do iclass=1,self%nclass
            ...
            reproduction_pel(iclass) = omega(iclass)*self%psi(iclass)*g_tot_pel*Nw(iclass)
            reproduction_ben(iclass) = (1._rk-omega(iclass))*self%psi(iclass)*g_tot_ben*Nw(iclass)
         end do
         ...
         total_reproduction = sum(reproduction_pel+reproduction_ben)
         ...
         do iprey = 1, self%nprey
            _SET_BOTTOM_ODE_(self%id_Nw_prey(iprey), (-prey_loss_pel(iprey) * Nw_prey(iprey)))
            _SET_BOTTOM_ODE_(self%id_Nw_prey(iprey), (-prey_loss_ben(iprey) * Nw_prey(iprey)))
         end do
...
end module mizer_size_structured_population_ben
