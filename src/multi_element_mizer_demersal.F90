...
!
! !INTERFACE:
module mizer_multi_element_demersal_population
!
! !DESCRIPTION:
...
! !PUBLIC DERIVED TYPES:
   type,extends(type_particle_model),public :: type_multi_element_demersal_population
      ...
      type (type_bottom_state_variable_id),         allocatable :: id_waste(:)      
      type (type_state_variable_id)                      :: id_waste_c
      type (type_state_variable_id)                      :: id_waste_n
      type (type_state_variable_id)                      :: id_waste_p
      type (type_state_variable_id)                      :: id_waste_s
      type (type_state_variable_id)                      :: id_o2
      type (type_state_variable_id)                      :: id_dic
      type (type_state_variable_id)                      :: id_dip
      type (type_state_variable_id)                      :: id_din
      type (type_state_variable_id)                      :: id_discard_c
      type (type_state_variable_id)                      :: id_discard_n
      type (type_state_variable_id)                      :: id_discard_p
      type (type_state_variable_id)                      :: id_discard_s
      ...
      type (type_horizontal_dependency_id),                    allocatable :: id_pelprey_c(:)
      ...
      type (type_horizontal_diagnostic_variable_id), allocatable:: id_fprey_c(:), id_fprey_n(:),id_fprey_p(:),id_fprey_s(:)
      real(rk)                                                  :: w_threshold
   ...
   end type type_multi_element_demersal_population
...
!
! !INPUT PARAMETERS:
   class (type_multi_element_demersal_population), intent(inout),target :: self
...
! !LOCAL VARIABLES:
   ...
   real (rk)          :: min_particle_mass, max_particle_mass
   real(rk)           :: c_ini
 ...
 !  class (type_weighted_sum), pointer :: total_pelprey_calculator
  ! class (type_depth_averaged_prey),  pointer :: depth_averaged_prey
   ...
   class (type_depth_averaged_class_demersal), pointer :: depth_averaged_class_demersal
...
!BOC
   ...
   call self%get_parameter(self%demersal, 'demersal', '', 'type of fish which defines resource spectrum characteristics', default=.false.)
...
!      call self%request_coupling(self%id_T_w_int, './T_w_integrator/result')
   ...
   if (self%demersal) then
       allocate (demersal_size_spectrum)
       call demersal_size_spectrum%parameters%set_integer('nsource', self%nprey)
       call demersal_size_spectrum%parameters%set_real('w_min', self%w_min/self%beta**2)
       call demersal_size_spectrum%parameters%set_real('w_max', self%w_min)
   end if
   ...
   allocate (self%id_fprey_c(self%nprey ))
   allocate (self%id_fprey_n(self%nprey ))
   allocate (self%id_fprey_p(self%nprey))
   allocate (self%id_fprey_s(self%nprey))
 ...
 !  allocate(total_pelprey_calculator)
   do iprey=1,self%nprey
      ...
      call self%register_state_dependency(self%id_prey_c(iprey), 'prey'//trim(strindex)//'_c', 'mmol C m-2', 'average encountered concentration of carbon in prey '//trim(strindex))
      call self%register_state_dependency(self%id_prey_n(iprey), 'prey'//trim(strindex)//'_n', 'mmol N m-2', 'average encountered concentration of nitrogen in prey '//trim(strindex))
      call self%register_state_dependency(self%id_prey_p(iprey), 'prey'//trim(strindex)//'_p', 'mmol P m-2', 'average encountered concentration of phosphorus in prey '//trim(strindex))
      call self%register_state_dependency(self%id_prey_s(iprey), 'prey'//trim(strindex)//'_s', 'mmol Si m-2', 'average encountered concentration of silicate in prey '//trim(strindex))
...
!      call self%register_model_dependency(self%id_prey(iprey),'prey'//trim(index))
 ...
 !      call depth_averaged_prey%set_variable_property(depth_averaged_prey%id_c, 'max_particle_mass', w_prey_max)          
       ...
       call demersal_size_spectrum%couplings%set_string('source'//trim(strindex), '../pelprey_c'//trim(strindex))      

       call self%register_diagnostic_variable(self%id_fprey_c(iprey),'fprey'//trim(strindex)//'c','mmol C/m^2/d',   'uptake of carbon in food source '//trim(strindex),    source=source_do_bottom)
       call self%register_diagnostic_variable(self%id_fprey_n(iprey),'fprey'//trim(strindex)//'n','mmol C/m^2/d',   'uptake of nitrogen in food source '//trim(strindex),    source=source_do_bottom)
       call self%register_diagnostic_variable(self%id_fprey_p(iprey),'fprey'//trim(strindex)//'p','mmol C/m^2/d',   'uptake of phosphorus in food source '//trim(strindex),    source=source_do_bottom)
       call self%register_diagnostic_variable(self%id_fprey_s(iprey),'fprey'//trim(strindex)//'s','mmol C/m^2/d',   'uptake of silicon in food source '//trim(strindex),    source=source_do_bottom)
...
!   call depth_integral%request_coupling('source', '../w_squarer/result')
   ...
   do iclass=1, self%nclass
      ...
      if (cannibalism) call self%set_variable_property(self%id_prey_c(iclass+self%nprey - self%nclass), 'particle_mass',self%w(iclass))
...
!      call depth_averaged_class%set_variable_property(depth_averaged_class%id_c,'particle_mass',self%w(iclass))
     ...
     ! call depth_averaged_class%request_coupling('int_w2', '../w2_integrator/result')
      ...
      if (cannibalism) then
        ...
        ! call self%request_coupling_to_model('prey'//trim(strindex2)//'_c',self%id_prey_c(self%nprey - self%nclass + iclass) ) 
      end if
...
!         write (strindex2,'(i0)') self%nprey - self%nclass + iclass
!         
!         call self%couplings%set_string('prey'//trim(strindex2)//'_c', './class'//trim(strindex))
!  !       call self%couplings%set_string('prey'//trim(strindex2)//'_c', './class'//trim(strindex))
! !        (self%id_prey_c(iprey), 'prey'//trim(strindex), standard_variables%total_carbon)
!  !       call self%couplings%set_string('ave_prey'//trim(strindex2), './class'//trim(strindex))
!      end if
  ...
  call self%register_state_dependency(self%id_dip,'dipp','mmol P/m^3','disssolved inorganic phosphorus')
   call self%register_state_dependency(self%id_din,'dinn','mmol N/m^3','dissolved inorganic nitrogen')

   call self%request_coupling_to_model(self%id_dip,   'excreted_phosphorus',   standard_variables%total_phosphorus)
   call self%request_coupling_to_model(self%id_din,   'excreted_nitrogen',   standard_variables%total_nitrogen)
...
!   call self%request_coupling_to_model(self%id_RPs,'RP','s')
   ...
   call self%register_state_dependency(self%id_dic,'O3c','mmol C/m^3','carbon dioxide')
   call self%register_state_dependency(self%id_o2,'O2o','mmol O_2/m^3','oxygen')
   call self%request_coupling_to_model(self%id_dic,   'respired_carbon',   standard_variables%total_carbon)
   call self%request_coupling_to_model(self%id_o2,   'oxygen_consumed',   standard_variables%total_carbon)
...
!   call register_waste('excreted_phosphorus', 'p', id_p=self%id_dip)
   ...
   call self%register_state_dependency(self%id_discard_c, 'discard_c', 'mmol C m-2', 'organic carbon discards')
   call self%register_state_dependency(self%id_discard_n, 'discard_n', 'mmol N m-2', 'organic nitrogen discards')
   call self%register_state_dependency(self%id_discard_p, 'discard_p', 'mmol P m-2', 'organic phosphorus discards')
   call self%register_state_dependency(self%id_discard_s, 'discard_s', 'mmol P m-2', 'organic silicate discards')
   ...
   call self%request_coupling_to_model(self%id_discard_s, 'discards', standard_variables%total_silicate)
...
!   call self%request_coupling_to_model(self%id_RPs,'RP','s')
   ...
   elseif (self%SRR == 3) then
      ...
      call self%register_horizontal_dependency(self%id_offset, 'prey_spectrum_offset', '-', 'offset of pelagic prey spectrum')
      call self%register_horizontal_dependency(self%id_slope, 'prey_spectrum_slope', '-', 'slope of pelagic prey spectrum')
      call self%request_coupling(self%id_offset, './demersal_size_spectrum/offset')
      call self%request_coupling(self%id_slope, './demersal_size_spectrum/slope')
   end if
 ...
 !  call self%register_state_dependency(self%id_O2o,'O2o','mmol O_2/m^3','oxygen')
   ...
   subroutine register_waste(name, composition, id_c, id_n, id_p, id_s, id_o2)
      ...
      type (type_state_variable_id), intent(inout), target, optional :: id_c, id_n, id_p, id_s, id_o2
 ...
 !     call self%add_child(waste, name//'_int', configunit=-1)
      if (present(id_c)) then
         call self%register_state_dependency(id_c, name//'_c', 'mmol C m-2', 'particulate organic carbon waste')
        ! call self%request_coupling(id_c, name//'_int/c_int')
      end if
      if (present(id_n)) then
         call self%register_state_dependency(id_n, name//'_n', 'mmol N m-2', 'particulate organic nitrogen waste')
      !   call self%request_coupling(id_n, name//'_int/n_int')
      end if
      if (present(id_p)) then
         call self%register_state_dependency(id_p, name//'_p', 'mmol P m-2', 'particulate organic phosphorus waste')
         call self%request_coupling(id_p, name//'_int/p_int')
      end if
      if (present(id_s)) then
         call self%register_state_dependency(id_s, name//'_s', 'mmol Si m-2','particulate silicate waste')
      !   call self%request_coupling(id_s, name//'_int/s_int')
      end if
      if (present(id_o2)) then
         call self%register_state_dependency(id_o2, name//'_o', 'mmol O_2 m-2','oxygen source')
...
!EOC
   ...
   subroutine after_coupling(self)
      class (type_multi_element_demersal_population),intent(inout) :: self
      ...
      do iprey=1, self%nprey
         ...
         elseif (w_p > 0) then
            ...
            ! Compute size-class-specific preference for current prey; Eq 4 in Hartvig et al. 2011 JTB, but note sigma typo confirmed by KH Andzersen
            ! This is a log-normal distribution of prey mass, scaled such that at optimum prey mass (=predator mass/beta), the preference equals 1.
            ! sigma is the standard deviation in ln mass units.
            do iclass=1, self%nclass
   ...
   subroutine do_bottom(self,_ARGUMENTS_DO_BOTTOM_)
      class (type_multi_element_demersal_population),intent(in) :: self
      ...
      _HORIZONTAL_LOOP_BEGIN_
         ...
         do iprey = 1, self%nprey
            ...
            elseif (self%feedback) then
               ...
               _SET_HORIZONTAL_DIAGNOSTIC_(self%id_fprey_c(iprey), prey_loss(iprey)*prey_c(iprey))
               _SET_HORIZONTAL_DIAGNOSTIC_(self%id_fprey_n(iprey), prey_loss(iprey)*prey_n(iprey))
               _SET_HORIZONTAL_DIAGNOSTIC_(self%id_fprey_p(iprey), prey_loss(iprey)*prey_p(iprey))
               _SET_HORIZONTAL_DIAGNOSTIC_(self%id_fprey_s(iprey), prey_loss(iprey)*prey_s(iprey))
            end if
         end do
         ...
         if (self%feedback) then
            _SET_BOTTOM_EXCHANGE_(self%id_o2,-self%resp_o2C*(1-self%alpha-self%alpha_eg)*sum(I_c*Nw))
            _SET_BOTTOM_EXCHANGE_(self%id_dic,(1-self%alpha-self%alpha_eg)*sum(I_c*Nw))
            _SET_BOTTOM_EXCHANGE_(self%id_din,(1-self%alpha-self%alpha_eg)*sum(I_n*Nw))
            _SET_BOTTOM_EXCHANGE_(self%id_dip,(1-self%alpha-self%alpha_eg)*sum(I_p*Nw))
            _SET_BOTTOM_EXCHANGE_(self%id_discard_c,sum(((self%alpha+self%alpha_eg)*I_c +  mu - g/(1-self%psi)          )*Nw) - R*self%w_min/g_per_mmol_carbon          + nflux(self%nclass)*(self%w(self%nclass)+self%delta_w(self%nclass))/g_per_mmol_carbon)
            _SET_BOTTOM_EXCHANGE_(self%id_discard_n,sum(((self%alpha+self%alpha_eg)*I_n + (mu - g/(1-self%psi))*self%qnc)*Nw) - R*self%w_min/g_per_mmol_carbon*self%qnc + nflux(self%nclass)*(self%w(self%nclass)+self%delta_w(self%nclass))/g_per_mmol_carbon*self%qnc)
            _SET_BOTTOM_EXCHANGE_(self%id_discard_p,sum(((self%alpha+self%alpha_eg)*I_p + (mu - g/(1-self%psi))*self%qpc)*Nw) - R*self%w_min/g_per_mmol_carbon*self%qpc + nflux(self%nclass)*(self%w(self%nclass)+self%delta_w(self%nclass))/g_per_mmol_carbon*self%qpc)
            _SET_BOTTOM_EXCHANGE_(self%id_discard_s,sum(I_s*Nw))
         end if
...
end module mizer_multi_element_demersal_population

!-----------------------------------------------------------------------
! Copyright Jorn Bruggeman/PML 2015-2017, modified by Helen Powley, 2023
!-----------------------------------------------------------------------
